...
<div x-data="{
    ...
    <div id="wizard-content-area" class="overflow-y-auto flex-1 px-6 py-8 bg-white">
        {{ $content }}
    </div>

    <!-- Navigation Buttons - Fixed at bottom -->
    <div
        class="sticky right-0 bottom-0 left-0 p-4 bg-white border-t border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <div class="flex justify-between items-center">
            <!-- Back Button -->
            <div>
                <x-buttons.button iconClass="ti ti-arrow-left" iconPosition="left" color="default" size="md"
                    x-show="currentStep > 1" @click="goToStep(currentStep > 1 ? currentStep - 1 : currentStep)">
                    Sebelumnya
                </x-buttons.button>
            </div>
            ...
            <div class="flex space-x-3">
                ...
                <x-buttons.button iconClass="ti ti-arrow-right" iconPosition="right" color="primary" size="md"
                    x-show="currentStep < totalSteps" @click="goToStep(currentStep + 1)">
                    Selanjutnya
                </x-buttons.button>

                <!-- Submit Button -->
                <x-buttons.button iconClass="ti ti-check" iconPosition="right" color="success" size="md"
                    x-show="currentStep === totalSteps" type="submit" @click="goToStep(currentStep + 1)">
                    Simpan Data
                </x-buttons.button>
            </div>
        </div>
