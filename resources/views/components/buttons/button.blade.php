...
@endif
    ...
    @if ($hasRipple)
        <span class="overflow-hidden absolute inset-0 rounded-md pointer-events-none" x-data="{ isRippling: false, x: 0, y: 0 }" @click="isRippling = true; x = $event.offsetX; y = $event.offsetY;" @animationend="isRippling = false">
            <span x-cloak x-show="isRippling" x-bind:style="'left: ' + x + 'px; top: ' + y + 'px'" class="absolute rounded-full bg-white/20 animate-ripple"></span>
        </span>
    @endif
